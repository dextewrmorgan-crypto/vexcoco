...
public class Finder extends Module {
   ...
   private final Setting<Boolean> onlyOnBedrock = this.sgGeneral
      .add(
         ((Builder)((Builder)((Builder)new Builder().name("only-on-bedrock")).description("Nur Spawner auf Bedrock-Höhe (Y=0) erkennen.")).defaultValue(true))
            .build()
      );
   ...
   private final Setting<Boolean> serverSideBypass = this.sgGeneral
      .add(
         ((Builder)((Builder)((Builder)new Builder().name("server-side-bypass")).description("Server-seitige Anti-ESP Umgehung.")).defaultValue(false)).build()
      );
   ...
   private final Setting<Boolean> detectOnBedrock = this.sgGeneral
      .add(
         ((Builder)((Builder)((Builder)new Builder().name("detect-on-bedrock")).description("Player auf Bedrock (Y <= -64) detektieren.")).defaultValue(true))
            .build()
      );
   ...
   public Finder() {
      super(AddonTemplate.CATEGORY, "Finder", "Markiert Chunks mit verdächtigen Aktivitäten (Spawner, Redstone, etc.).");
   }
   ...
   private void onTick(Post event) {
      if (this.mc.world != null && this.mc.player != null) {
         try {
            if (this.mc.world.method_75260() % 20L == 0L) {
   ...
   private void scanForHiddenSpawners() {
      if (this.mc.world != null && this.mc.player != null) {
         try {
            ChunkPos playerChunk = this.mc.player.method_31476();
            int range = 5;

            for (int dx = -range; dx <= range; dx++) {
               for (int dz = -range; dz <= range; dz++) {
                  ChunkPos chunkPos = new ChunkPos(playerChunk.x + dx, playerChunk.z + dz);
                  if (this.mc.world.getChunkManager().method_12123(chunkPos.x, chunkPos.z)) {
                     WorldChunk chunk = this.mc.world.method_8497(chunkPos.x, chunkPos.z);
                     if (chunk != null) {
                        ...
                        for (PlayerEntity player : this.mc.world.method_18456()) {
                           if (player != null && player.method_31476().equals(chunkPos)) {
   ...
   private void checkEntity(Entity entity) {
      if (entity != null && this.mc.world != null) {
         try {
            BlockPos pos = entity.method_24515();
            BlockPos belowPos = pos.down();
            if (this.mc.world.method_8320(belowPos).method_27852(Blocks.DEEPSLATE) || this.mc.world.method_8320(belowPos).method_27852(Blocks.BEDROCK)) {
      ...
      if (entity != null) {
         try {
            if (!this.flaggedEntities.contains(entity.method_5628())) {
               ...
               ChunkPos chunkPos = new ChunkPos(entity.method_24515());
   ...
   private void addBaseChunk(BlockPos pos) {
      try {
         BlockPos aboveBasePos = new BlockPos(pos.method_10263(), pos.method_10264() + 5, pos.method_10260());
   ...
   private boolean isRedstoneActive(BlockState state) {
      try {
         return state.method_27852(Blocks.REDSTONE_BLOCK)
            || state.method_27852(Blocks.REDSTONE_TORCH)
            || state.method_27852(Blocks.REPEATER)
            || state.method_27852(Blocks.COMPARATOR)
            || state.method_27852(Blocks.PISTON)
            || state.method_27852(Blocks.STICKY_PISTON)
            || state.method_27852(Blocks.HOPPER)
            || state.method_27852(Blocks.DISPENSER)
            || state.method_27852(Blocks.DROPPER)
            || state.method_27852(Blocks.REDSTONE_WIRE);
      } catch (Exception e) {
         return false;
      }
   }
   ...
   private void handleBlockEntityUpdate(BlockEntityUpdateS2CPacket packet) {
      try {
         BlockPos pos = packet.getPos();
         ChunkPos chunkPos = new ChunkPos(pos);
         if (this.mc.world != null && this.mc.world.method_8321(pos) instanceof MobSpawnerBlockEntity spawner) {
      ...
      if (this.mc.world != null && this.mc.player != null) {
         try {
            ...
            this.playerPositions.removeIf(pos -> {
               for (PlayerEntity player : this.mc.world.method_18456()) {
                  if (player != null) {
                     BlockPos playerPos = new BlockPos((int)player.method_23317(), (int)player.method_23318(), (int)player.method_23321());
   ...
   private void scanForSpawnersDirect() {
      if (this.mc.world != null && this.mc.player != null) {
         try {
            ChunkPos playerChunkPos = this.mc.player.method_31476();
            int range = 3;
               ...
               if (blockEntity instanceof MobSpawnerBlockEntity spawner) {
                  ...
                  if ((!(Boolean)this.onlyOnBedrock.get() || spawnerY == 0 || (Boolean)this.packetAntiEspBypass.get() && spawnerY <= 0)
                     ...
                     this.spawnerChunks.removeIf(existingChunkPos -> {
                        try {
                           WorldChunk existingChunk = this.mc.world.method_8497(existingChunkPos.x, existingChunkPos.z);
               ...
               if (player != null) {
                  ...
                  if (shouldDetect) {
                     ...
                     this.playerChunks.removeIf(existingChunk -> {
                        for (BlockEntity blockEntity : this.mc.world.method_8497(existingChunk.x, existingChunk.z).getBlockEntities().values()) {
                     ...
                     if ((Boolean)this.packetAntiEspBypass.get()) {
                        new Thread(() -> {
                           try {
                              Thread.sleep(((Integer)this.scanDelay.get()).intValue());
   ...
   private void validatePlayerPosition(BlockPos pos, ChunkPos chunkPos) {
      if (this.mc.world != null) {
         try {
            if (this.mc.world.method_8320(pos).method_27852(Blocks.BEDROCK)
               || this.mc.world.method_8320(pos.down()).method_27852(Blocks.BEDROCK)
               || this.mc.world.method_8320(pos).method_27852(Blocks.DEEPSLATE)
               || this.mc.world.method_8320(pos.down()).method_27852(Blocks.DEEPSLATE)) {
               return;
            }
   ...
   private void onChunkData(ChunkDataEvent event) {
      ChunkPos chunkPos = event.chunk().method_12004();
      ...
      if ((Boolean)this.antiEspMode.get()) {
         for (int i = 0; i < this.maxRetries.get(); i++) {
            ...
            new Thread(() -> {
               try {
                  Thread.sleep((Integer)this.scanDelay.get() * (retry + 1));
   ...
   private void scanChunkStandard(WorldChunk chunk, int retry) {
      if (chunk != null) {
         ...
         try {
            ...
            for (BlockEntity blockEntity : chunk.getBlockEntities().values()) {
               if (blockEntity instanceof MobSpawnerBlockEntity spawner
                  && (!(Boolean)this.onlyOnBedrock.get() || spawner.method_11016().method_10264() == 0)
                  && (!(Boolean)this.strictValidation.get() || this.isValidSpawner(spawner))) {
      ...
      if (chunk != null) {
         ...
         try {
            ...
            for (BlockEntity blockEntity : chunk.getBlockEntities().values()) {
               totalBlocks++;
               if (blockEntity != null) {
                  if (blockEntity.getCachedState().method_26225() && blockEntity.getCachedState().method_31709()) {
                     suspiciousBlocks++;
                  }

                  if (blockEntity instanceof MobSpawnerBlockEntity && (!(Boolean)this.onlyOnBedrock.get() || blockEntity.getPos().method_10264() == 0)) {
                     this.spawnerChunks.add(pos);
                     this.pendingChunks.remove(pos);
                     return;
                  }
            ...
            if (totalBlocks > 0 && suspiciousBlocks >= totalBlocks * 0.3) {
               this.spawnerChunks.add(pos);
               this.pendingChunks.remove(pos);
               return;
            }
   ...
   private boolean hasSpawnerIndicators(WorldChunk chunk) {
      try {
         ...
         for (BlockEntity blockEntity : chunk.getBlockEntities().values()) {
            totalEntities++;
            if (blockEntity != null) {
               if (!(blockEntity instanceof EnderChestBlockEntity) && !blockEntity.getCachedState().method_27852(Blocks.SHULKER_BOX)) {
                  if ((
                        blockEntity.getCachedState().method_27852(Blocks.CHEST)
                           || blockEntity.getCachedState().method_27852(Blocks.TRAPPED_CHEST)
                           || blockEntity.getCachedState().method_27852(Blocks.BARREL)
                     )
                     && blockEntity.getPos().method_10264() < 0) {
                     this.storageChunks.add(chunk.method_12004());
                  }
               } else if (blockEntity.getPos().method_10264() < 0) {
               ...
               if (blockEntity instanceof MobSpawnerBlockEntity && blockEntity.getPos().method_10264() < 0) {
                  return true;
               }
            }
         }
   ...
   private boolean isPlayerNearChunk(ChunkPos pos) {
      ...
      double distance = Math.sqrt(
         Math.pow(this.mc.player.method_23317() - (pos.x * 16 + 8), 2.0) + Math.pow(this.mc.player.method_23321() - (pos.z * 16 + 8), 2.0)
      );
   ...
   private boolean isValidSpawner(MobSpawnerBlockEntity spawner) {
      try {
         if (spawner == null) {
            ...
            BlockPos pos = spawner.method_11016();
            if (pos != null && this.mc.world != null) {
               return !this.mc.world.method_8320(pos).method_27852(Blocks.SPAWNER) ? false : spawner.getLogic() != null;
            } else {
               return false;
            }
         }
   ...
   private void renderChunkBox(Render3DEvent event, ChunkPos chunkPos, SettingColor color) {
      ...
      if (this.renderMode.get() == Finder.RenderMode.Flat) {
         y1 = ((Integer)this.flatRenderY.get()).intValue();
         y2 = y1 + 1.0;
      } else {
         y1 = this.mc.world.method_31607();
         y2 = 320.0;
      }
         ...
         for (int dx = -radius; dx <= radius; dx++) {
            for (int dy = -radius; dy <= radius; dy++) {
               for (int dz = -radius; dz <= radius; dz++) {
                  ...
                  if (state.method_27852(Blocks.SPAWNER)) {
                     if (world.method_8321(pos) instanceof MobSpawnerBlockEntity spawner) {
      ...
      if (chunk != null) {
         try {
            ...
            for (BlockEntity be : chunk.getBlockEntities().values()) {
               ...
               if (be instanceof EnderChestBlockEntity || state.method_27852(Blocks.SHULKER_BOX)) {
                  playerMadeCount++;
                  storageCount++;
                  if (be.getPos().method_10264() < 0) {
                     this.greenBaseChunks.add(chunkPos);
                  }
               } else if (state.method_27852(Blocks.CHEST) || state.method_27852(Blocks.TRAPPED_CHEST) || state.method_27852(Blocks.BARREL)) {
                  storageCount++;
               } else if (state.method_27852(Blocks.HOPPER) || state.method_27852(Blocks.DROPPER)) {
                  playerMadeCount++;
                  if (be.getPos().method_10264() < 0) {
                     this.greenBaseChunks.add(chunkPos);
                  }
   ...
   private void applyEntitySpawnAntiEsp(EntitySpawnS2CPacket spawnPacket) {
      ...
      if (entityId != -1) {
         ...
         if (stateBelow != null && (stateBelow.method_27852(Blocks.DEEPSLATE) || stateBelow.method_27852(Blocks.BEDROCK))) {
   ...
   private BlockState getBlockState(BlockPos pos) {
      return this.mc.world != null ? this.mc.world.method_8320(pos) : null;
   }
